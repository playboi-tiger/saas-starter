...
export const listProjectsTool = {
  ...
  config: {
    title: "List projects",
    description:
      "Lists the user's projects. Use this whenever you need a `projectId` for another tool. Returns an array of {id, name, domain, description, url}; pass the `id` value as `projectId`.",
  ...
  handler: async (_args: Record<string, never>, context: ToolContext) => {
    ...
    const lines =
      projects.length === 0
        ...
        : projects.map(
            (p) =>
              `- ${p.id}  ${p.name}${p.domain ? ` (${p.domain})` : ""}${p.organization ? `  organization:${p.organization} [${p.organizationId}]` : ""}`,
          );
    return mcpResponse({
