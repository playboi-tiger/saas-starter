...
export const getProjectContextTool = {
  ...
  config: {
    ...
    description:
      "Reads a project's shared memory: business overview, current goal, positioning, writing preferences, and custom sections. Call this to ground context in what the user already specified. Sections listed as missing are the ones worth filling with update_project_context.",
...
export function buildUpdateProjectContextTool(author: ContextAuthor) {
  return {
    ...
    config: {
      ...
      description:
        "Writes to a project's shared memory so the app, chat assistant, and other agents see it. Send a list of patch ops; sections are prose (~4,000 chars max). Confirm facts with the user before storing them.",
      inputSchema: updateInputSchema,
      outputSchema: contextOutputSchema,
      annotations: {
        readOnlyHint: false,
        openWorldHint: false,
        // The op union includes section deletions.
